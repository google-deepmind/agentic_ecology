...
1. **Label Selection/Definition (Level 2):** Allows the user to select an
...
1. **Ranking and Filtering Configuration (Level 3):** Allows the user to
...
1. **Row Visualization and Annotation (Level 4):** Allows the user to visualize
...
- **Pattern:** Always define a clear, functional width for media players
  ...
  table column has sufficient space and prevents wrapping (e.g., `white-space: nowrap;`).
...
- **NumPy Types**: AI/ML libraries (like USearch or NumPy itself) often return
  ...
  support these and will raise a `TypeError: Object of type ... is not JSON serializable`.
