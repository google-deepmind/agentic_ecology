...
To install `google-deepmind/agentic_ecology` skills locally, execute `npx skills
add google-deepmind/agentic_ecology -y -a <your_name>`. If you are unsure what
value to use for `<your_name>`, list accepted values programmatically with `curl
-s https://registry.npmjs.org/skills/latest | jq '[.keywords[] | select(. as $k
...
# Install google-deepmind/agentic_ecology
...
reference file [pyproject.toml](pyproject.toml) and copy [uv.lock](uv.lock) from
this skill's `references/` directory into the target workspace root. This equips
the project with the complete dependency configuration and pinned lockfile:

```bash
# Overwrite pyproject.toml and copy uv.lock from skill references
cp <skill_dir>/references/pyproject.toml .
cp <skill_dir>/references/uv.lock .
```
...
Copy the reference [AGENTS.md](AGENTS.md) from this skill's `references/`
directory into the appropriate location in the target project workspace to
preserve essential workspace rules (such as macOS TensorFlow/PyArrow deadlock
prevention, Linux PyTorch/TensorFlow import order, and SQL log suppression
filters).
...
Copy the reference verification script [verify_env.py](verify_env.py) from this
skill's `references/` directory into `agent_workspace/` and execute it via `uv
run python` to confirm that all core libraries are available and import safely
without deadlocks:

```bash
cp <skill_dir>/references/verify_env.py agent_workspace/
