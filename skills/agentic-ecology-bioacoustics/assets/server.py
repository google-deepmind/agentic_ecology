...
# ==============================================================================
# SECTION 1: Database Inspection & Performance Utilities
# ==============================================================================
...
class CachedHopliteDB:
    ...
    **How?**
    ...
    def get_window_annotations(self, window_id: int, label: str | None = None) -> Any:
...
class HopliteDatabaseAdapter(BaseDatabaseAdapter):
    ...
    def get_labels(self, database_id: str) -> list[dict[str, Any]]:
        ...
        unique_labels = sorted(list(set(labels_pos.keys()) | set(labels_neg.keys())))
        return [
            {
                "name": name,
    ...
    def search(
        ...
        # descending.
        if (query_uri and mode == SearchMode.QUERY) or mode == SearchMode.CLASSIFIER:
    ...
    def get_media_preview(self, database_id: str, item_id: int) -> tuple[bytes, str]:
    ...
    def get_query_preview(self, database_id: str, query_uri: str) -> tuple[bytes, str]:
    ...
    def _read_audio_window(
        ...
        if base_path is None:
            checked_paths = [g.get("base_path") for g in audio_sources_meta.audio_globs]
    ...
    def _compute_spectrogram_from_audio(self, data: np.ndarray, sr: float) -> bytes:
    ...
    def _retrieve_user_annotations(
        ...
        annotations_filter = config_dict.create(eq=dict(label=label, provenance="user"))
    ...
    def _train_active_learning_classifier(
        ...
        scores = {wid: float(probabilities[i, 0]) for i, wid in enumerate(window_ids)}

        metrics = {
            "accuracy": (
                sanitize_float(eval_scores.get("top1_acc", 1.0)) if eval_scores else 1.0
            ),
