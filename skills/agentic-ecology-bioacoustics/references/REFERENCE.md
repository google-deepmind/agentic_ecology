...
- **Window**: Represents a segment of audio.
  ...
  - `offsets`: `list[float]` (start and end times in seconds, e.g., `[0.0, 5.0]`).
...
1. Retrieve `audio_sources` metadata: `audio_sources_meta = db.get_metadata("audio_sources")`.
1. Retrieve the matching deployment to find the corresponding project name:
   `deployment = db.get_deployment(recording.deployment_id)` and
   `dataset_name = deployment.project`.
1. Match the `dataset_name` to the correct `AudioSourceConfig` in
...
1. Combine with the recording's filename: `os.path.join(base_path, recording.filename)`.
...
1. Compute the PCEN mel-spectrogram matrix using
   `perch_hoplite.agile.embedding_display.pcen_melspec_display(data, sample_rate_hz=sr)`.
1. Normalize the resulting spectrogram values to `[0, 1]`.
1. Flip the transposed spectrogram matrix vertically: `np.flipud(spec.T)`.
1. Map the normalized matrix values through a colormap (e.g.
...
- **SearchResult Schema**: The results yielded by iterating over
  ...
  - *(Note: Historical references might use `result.key` and
    ...
    `result.sort_score`)*.
- **Xeno-Canto & Audio Loading**: The utility function `load_audio(path, target_sample_rate)` (imported from `perch_hoplite.audio_io`) is the
...
- **Retrieving Split IDs & Dataset Size**: Call
  `data_manager.get_train_test_split()` (which returns a tuple of `(train_ids, eval_ids)`) to inspect splits and count total annotated examples. Do **not**
